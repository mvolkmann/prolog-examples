...
eval(a(Name, Value), VT0, VT) :- lookup(Value, V), vtables_put(Name, V).

% This calls a function, but does not use its return value.
eval(c(Name, Args), VT0, VT) :- process_call(Name, Args).

% This stores a function definition in the current vtable.
eval(f(Name, Params, Stmts), VT0, VT) :- vtables_put(Name, [Params, Stmts]).

% This evaluates all the statements in a program.
eval(pr(Stmts), VT0, VT) :- maplist(eval, Stmts).

% This prints a value to stdout.
eval(p(Value), VT0, VT) :- lookup(Value, V), writeln(V).
