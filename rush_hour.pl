...
:- initialization
  ...
  Cars = [
    car(l, (0, 0), (1, 0)), % l for lime green
    car(p, (0, 1), (0, 3)), % p for purple
    car(x, (1, 2), (2, 2)), % target car has color x
    car(o, (0, 4), (0, 5)), % o for orange
    car(b, (3, 1), (3, 3)), % b for blue
    car(c, (4, 4), (5, 4)), % c for cyan (light blue)
    car(g, (2, 5), (4, 5)), % g for green
    car(y, (5, 0), (5, 2))  % y for yellow
  ],
  Exit = (5, 2),
  Size = 6,
  Board = board(Size, Size, Exit, Cars),
  % This works, the solution is too long and contains many repetitive moves.
  solve_dfs_print(Board),
